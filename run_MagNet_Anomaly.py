...
f_node, e_node, weights = A.row, A.col, A.data
size = G.number_of_nodes()
L = utils.hermitian_decomp_sparse(f_node, e_node, size, edge_weight=weights)
L_real = utils.cnv_sparse_mat_to_coo_tensor(L.real)
L_img = utils.cnv_sparse_mat_to_coo_tensor(L.imag)
...
utils.write_csv_header('./tmp/res/Anomaly_full_rounds/full_MagNet.csv')
for lr in [0.001, 0.005, 0.01, 0.05]:
    ...
    tpr_1 = []
    auc = []
    csvFileName =  './tmp/res/Anomaly_' + graph_name + '/' + str(hidden_dimension) + ' ' + strLr + '_MagNet.csv'
    ...
    for round in range(1, 11):
        ...
        with open('./tmp/res/Anomaly_full_rounds/full_MagNet.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(csvData)
