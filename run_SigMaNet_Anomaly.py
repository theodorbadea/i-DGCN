...
weights = torch.tensor(A.data)
edges = torch.Tensor([A.row, A.col])
edge_index, norm_real, norm_imag = utils.process_magnetic_laplacian(edge_index=edges, gcn=True, net_flow='-N',\
                                    x_real=signals, edge_weight=weights, normalization='sym', return_lambda_max=False)
L_real = scipy.sparse.coo_matrix((norm_real, (edge_index[0], edge_index[1])), shape=(G.number_of_nodes(), G.number_of_nodes()), dtype=np.float32)
L_imag = scipy.sparse.coo_matrix((norm_imag, (edge_index[0], edge_index[1])), shape=(G.number_of_nodes(), G.number_of_nodes()), dtype=np.float32)
L_real = utils.cnv_sparse_mat_to_coo_tensor(L_real)
L_img = utils.cnv_sparse_mat_to_coo_tensor(L_imag)
...
utils.write_csv_header('./tmp/res/Anomaly_full_rounds/full_SigMaNet.csv')
for lr in [0.001, 0.005, 0.01, 0.05]:
    ...
    tpr_1 = []
    auc = []
    csvFileName =  './tmp/res/Anomaly_' + graph_name + '/' + str(hidden_dimension) + ' ' + strLr + '_SigMaNet.csv'
    ...
    for round in range(1, 11):
        ...
        model = MagNet(size_in=size_in, size_out=size_out, hidden_dim=hidden_dimension, K=1, nb_layers=num_layers, droprate=0.5)                            
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            optimizer.zero_grad()
            output = model(signals, L_real, L_img)
        ...
        model.eval()
        with torch.no_grad():
            outEval = model(signals, L_real, L_img)
        ...
        with open('./tmp/res/Anomaly_full_rounds/full_SigMaNet.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(csvData)
