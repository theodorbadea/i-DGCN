...
import utils
from magnet_link import ChebNet_Edge
...
for task in ['existence']:
        ...
        for num_filter in [16, 32, 64]:
            for layer in [2, 4, 8]:
                ...
                for i in range(10):
                    ...
                    ########################################
                    model = ChebNet_Edge(in_c=X_real.size(-1), L_norm_real=L_real, L_norm_imag=L_img, num_filter=num_filter, K=1, label_dim=2, layer=layer, activation=True, dropout=dropout)
    ...
    for i in range(10):
        ...
        model = ChebNet_Edge(in_c=X_real.size(-1), L_norm_real=L_real, L_norm_imag=L_img, num_filter=best_error_model_num_filter, K=1, label_dim=2, layer=best_error_model_layer, activation=True, dropout=dropout)
        model = model.to(device)
        ...
        model = ChebNet_Edge(in_c=X_real.size(-1), L_norm_real=L_real, L_norm_imag=L_img, num_filter=best_acc_model_num_filter, K=1, label_dim=2, layer=best_acc_model_layer, activation=True, dropout=dropout)
        model = model.to(device)
